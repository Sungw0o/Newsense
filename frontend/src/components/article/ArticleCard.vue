...
const handleClick = () => {
  emit('click', props.article.articleId)
}
</script>
  ...
  >
    <div>
      <div class="flex items-center gap-2 mb-4">
        ...
        <span class="text-xs text-slate-400 select-none">{{ article.estimatedMinutes }}분 소요</span>
      </div>
    ...
    <div class="flex items-center justify-between border-t border-white/5 pt-4 mt-auto">
      ...
      <span class="text-xs text-slate-500 font-light select-none">{{ article.publishedAt }}</span>
    </div>
  </div>
</template>
