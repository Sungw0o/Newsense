...
public class NaverSearchNewsApiCollector {

    private static final String SOURCE_KEY = "naver-search";
    private static final String SOURCE_NAME = "네이버 뉴스 검색";
    ...
    public CrawlRunResult collect() {
        if (properties.naverClientId().isBlank() || properties.naverClientSecret().isBlank()) {
    ...
    private List<ArticleCandidate> fetchCandidates() {
        String query = URLEncoder.encode("경제 금융 투자 정책 산업", StandardCharsets.UTF_8);
        JsonNode response = RestClient.create().get()
                .uri(API_URL + "?query=" + query + "&display=" + properties.maxItemsPerSource() + "&sort=date")
                .header("X-Naver-Client-Id", properties.naverClientId())
                .header("X-Naver-Client-Secret", properties.naverClientSecret())
        ...
        for (JsonNode item : response.path("items")) {
            ...
            String url = item.path("originallink").asText(item.path("link").asText());
    ...
    private CrawlRunResult saveCandidates(List<ArticleCandidate> candidates) {
        ...
        for (ArticleCandidate candidate : candidates) {
            try {
                ...
                failed++;
                log.warn("Naver search news crawl failed: url={}", candidate.sourceUrl(), exception);
            }
        }
